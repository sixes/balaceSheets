...
export default function SalesSheet({ name, data, onChange, settings, setSettings }: Props) {
  ...
  const columns = useMemo(
    () => [
      { 
        ...
        width: 80, 
        editable: false,
        valueFormatter: (params) => params.value || '' // Ensure string rendering
      },
      { 
        headerName: '日  期', 
        field: 'date', 
      ...
      { field: 'id', hide: true }, // Explicitly hide id to prevent rendering
    ],
    []
  );
  ...
  useEffect(() => {
    if (gridRef.current?.api && data?.rows) {
      ...
      console.log(`Set pinned bottom rows for SalesSheet:`, pinnedBottomRow);
    }
  }, [pinnedBottomRow, data?.rows]);

  const handleGridReady = (params: any) => {
    gridRef.current = params;
    params.api.resetColumnState(); // Reset to ensure id is hidden
    params.api.setGridOption('pinnedBottomRowData', pinnedBottomRow);
    params.api.sizeColumnsToFit();
    params.api.redrawRows();
    console.log(`Grid ready for SalesSheet, columns sized, pinned rows set:`, pinnedBottomRow);
  };
  ...
  return (
    ...
    >
      <div className="sticky-title" style={{ flex: '0 0 auto' }}>
        <div style={{ fontWeight: 'bold', fontSize: '16px', marginBottom: '8px' }}>銷   貨</div>
        <div>
          公司名稱:{' '}
          <input
        ...
        >
          <AgGridReact
            ...
            onFirstDataRendered={(params) => {
              ...
              params.api.setGridOption('pinnedBottomRowData', pinnedBottomRow);
              params.api.redrawRows();
              console.log(`First data rendered for SalesSheet, columns sized, pinned rows:`, pinnedBottomRow);
            }}
          />
        </div>
      </div>
