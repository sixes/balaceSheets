...
export default function DirectorSheet({ name, data, onChange, settings, setSettings }: Props) {
  ...
  useEffect(() => {
    if (gridRef.current?.api && data?.rows) {
      ...
      console.log(`Set pinned bottom rows for DirectorSheet:`, pinnedBottomRow);
    }
  ...
  const handleGridReady = (params: any) => {
    ...
    console.log(`Grid ready for DirectorSheet, columns sized, pinned rows set:`, pinnedBottomRow);
  };
  ...
  return (
    ...
    >
      <div className="sticky-title" style={{ flex: '0 0 auto' }}>
        <div style={{ fontWeight: 'bold', fontSize: '16px', marginBottom: '8px' }}>董   事   往   來</div>
        <div>
          公司名稱:{' '}
          <input
        ...
        >
          <AgGridReact
            ...
            onFirstDataRendered={(params) => {
              ...
              params.api.setGridOption('pinnedBottomRowData', pinnedBottomRow);
              params.api.redrawRows();
              console.log(`First data rendered for DirectorSheet, columns sized, pinned rows:`, pinnedBottomRow);
            }}
          />
        </div>
      </div>
