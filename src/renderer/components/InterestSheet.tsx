...
export default function InterestSheet({ name, data, onChange, settings, setSettings }: Props) {
  ...
  useEffect(() => {
    if (gridRef.current?.api) {
      ...
      console.log('Grid focus and editing cleared for InterestSheet');
    }
    return () => {
      if (gridRef.current?.api) {
        gridRef.current.api.stopEditing();
        gridRef.current.api.clearFocusedCell();
        console.log('Grid cleaned up for InterestSheet');
      }
    };
  }, []);
  ...
  const { rows, pinnedBottomRow } = useMemo(() => {
    ...
    const calculatedRows = rowsArray.map((row, index) => ({
      ...row,
      no: row.no || index + 1,
      id: row.id || `interest-row-${index + 1}`,
    }));
    ...
    console.log('InterestSheet rows:', calculatedRows);
    console.log('InterestSheet pinnedBottomRow:', pinnedBottomRow);
  ...
  useEffect(() => {
    if (gridRef.current?.api && data?.rows) {
      ...
      console.log(`Set pinned bottom rows for InterestSheet:`, pinnedBottomRow);
    }
  ...
  const handleGridReady = (params: any) => {
    ...
    console.log(`Grid ready for InterestSheet, columns sized, pinned rows set:`, pinnedBottomRow);
  };
  ...
  return (
    ...
    >
      <div className="sticky-title" style={{ flex: '0 0 auto' }}>
        <div style={{ fontWeight: 'bold', fontSize: '16px', marginBottom: '8px' }}>利   息   收   入</div>
        <div>
          公司名稱:{' '}
          <input
        ...
        >
          <AgGridReact
            ...
            onCellValueChanged={(params) => {
              ...
              console.log('InterestSheet onChange:', newData);
              onChange(newData);
              if (gridRef.current?.api) {
            ...
            onFirstDataRendered={(params) => {
              ...
              params.api.setGridOption('pinnedBottomRowData', pinnedBottomRow);
              params.api.redrawRows();
              console.log(`First data rendered for InterestSheet, columns sized, pinned rows:`, pinnedBottomRow);
            }}
          />
        </div>
      </div>
