...
export default function PayableSheet({ name, data, onChange, settings, setSettings }: Props) {
  ...
  useEffect(() => {
    if (gridRef.current?.api) {
      ...
      console.log('Grid focus and editing cleared for PayableSheet');
    }
    return () => {
      if (gridRef.current?.api) {
        gridRef.current.api.stopEditing();
        gridRef.current.api.clearFocusedCell();
        console.log('Grid cleaned up for PayableSheet');
      }
    };
  }, []);
  ...
  const { rows, pinnedBottomRow } = useMemo(() => {
    ...
    console.log('PayableSheet rows:', calculatedRows);
    console.log('PayableSheet pinnedBottomRow:', pinnedBottomRow);
        ...
        >
          <AgGridReact
            ...
            onCellValueChanged={(params) => {
              ...
              console.log('PayableSheet onChange:', newData);
              onChange(newData);
              if (gridRef.current?.api) {
