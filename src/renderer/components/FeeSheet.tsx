...
export default function FeeSheet({ name, data, onChange, settings, setSettings }: Props) {
  ...
  useEffect(() => {
    if (gridRef.current?.api) {
      ...
      console.log('Grid focus and editing cleared for FeeSheet');
    }
    return () => {
      if (gridRef.current?.api) {
        gridRef.current.api.stopEditing();
        gridRef.current.api.clearFocusedCell();
        console.log('Grid cleaned up for FeeSheet');
      }
    };
  }, []);
  ...
  const { rows, pinnedBottomRow } = useMemo(() => {
    ...
    console.log('FeeSheet rows:', calculatedRows);
    console.log('FeeSheet pinnedBottomRow:', pinnedBottomRow);
  ...
  useEffect(() => {
    if (gridRef.current?.api && data?.rows) {
      ...
      console.log(`Set pinned bottom rows for FeeSheet:`, pinnedBottomRow);
    }
  ...
  const handleGridReady = (params: any) => {
    ...
    console.log(`Grid ready for FeeSheet, columns sized, pinned rows set:`, pinnedBottomRow);
  };
  ...
  return (
    ...
    >
      <div className="sticky-title" style={{ flex: '0 0 auto' }}>
        <div style={{ fontWeight: 'bold', fontSize: '16px', marginBottom: '8px' }}>銀   行   費   用</div>
        <div>
          公司名稱:{' '}
          <input
        ...
        >
          <AgGridReact
            ...
            onCellValueChanged={(params) => {
              ...
              console.log('FeeSheet onChange:', newData);
              onChange(newData);
              if (gridRef.current?.api) {
            ...
            onFirstDataRendered={(params) => {
              ...
              params.api.setGridOption('pinnedBottomRowData', pinnedBottomRow);
              params.api.redrawRows();
              console.log(`First data rendered for FeeSheet, columns sized, pinned rows:`, pinnedBottomRow);
            }}
          />
        </div>
      </div>
